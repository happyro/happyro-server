...
void clif_navigation_teleport_config( const map_session_data* sd ){
	clif_configuration_value(sd, CONFIG_NAVIGATION_TELEPORT_ALLOWED, happyro_navigation_teleport_allowed(sd));
...
void clif_parse_happyro_npc_availability( int32 fd, map_session_data* sd ){
	...
	for (uint16 i = 0; i < packet->count; ++i) {
		WFIFOB(fd, 10 + i) = happyro_navigation_npc(sd, packet->npcs[i].map, packet->npcs[i].npcX,
...
void clif_parse_happyro_npc_teleport( int32 fd, map_session_data* sd ){
	...
	const auto result = happyro_npc_teleport(sd, packet->map, packet->npcX, packet->npcY, packet->npcClass);
	clif_happyro_npc_teleport_result(fd, packet->requestId, static_cast<e_happyro_npc_teleport_result>(result.code),
		result.map.c_str(), result.x, result.y, result.cooldown);
}
...
void clif_parse_happyro_map_teleport( int32 fd, map_session_data* sd ){
	...
	const auto result = happyro_map_teleport(sd, packet->map, packet->x, packet->y);
	clif_happyro_map_teleport_result(fd, packet->requestId, static_cast<e_happyro_map_teleport_result>(result.code),
		result.map.c_str(), result.x, result.y, result.cooldown);
}
