...
void SkillAsuraStrike::calculateSkillRatio(const Damage* wd, const block_list* src, const block_list* target, uint16 skill_lv, int32& base_skillratio, int32 mflag) const {
	...
	int64 ratio = static_cast<int64>(base_skillratio) + 700 + static_cast<int64>(sstatus->sp) * 10;
#ifdef RENEWAL
	if (wd->miscflag&1)
		ratio *= 2; // More than 5 spirit balls active
#endif
	base_skillratio = cap_value(ratio, INT_MIN, 500000); //We stop at roughly 50k SP for overflow protection
}
